...
export default function AdminContentPage() {
  ...
  const [venues, setVenues] = useState([]); // danh sách sân thuộc 1 owner
  ...
  useEffect(() => {
    ...
    async function loadOwners() {
      ...
      try {
        ...
        // Tự chọn owner đầu tiên nếu có
        if (list.length > 0) {
          const first = list[0];
          setSelectedOwnerId(first.id);

          const venueList = first.venues || [];
          setVenues(venueList);
  ...
  // ---- Khi chọn chủ sân → tải danh sách sân của chủ đó ----
  useEffect(() => {
    const owner = owners.find(o => o.id === selectedOwnerId);
    const list = owner?.venues || [];

    setVenues(list);

    if (!list.some(v => v.id === selectedVenueId)) {
    ...
    async function loadContent() {
      ...
      try {
        const res = await apiFetch(
          `/admin/venues/${selectedVenueId}/content`
        );
        const payload =
  ...
  const uploadImage = async (sectionKey, file, options = {}) => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/admin/venues/${selectedVenueId}/content/upload-image`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSaving(true);
      await apiFetch(`/admin/venues/${selectedVenueId}/content`, {
        method: "PUT",
        body: content,
      });
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {loadingOwners ? (
                <option>Đang tải chủ sân...</option>
              ) : owners.length === 0 ? (
                <option>Không có chủ sân</option>
              ) : (
                owners.map(owner => (
            ...
            >
              {venues.length === 0 ? (
                <option>Chủ sân này chưa có sân</option>
              ) : (
                venues.map(v => (
